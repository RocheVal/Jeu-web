...
var player = {
    init : function(){
      ...
      /*
      Tentative d'amélioration de l'affichage
      J'ai essayé différentes choses mais soit le vaisseau est invisible soit le jeu pante (avec un mauvais argument visiblement)
      Même avec l'aide de quelques collègues je n'ai pas réussi à trouver ce qu'il me manquait.
      J'ai preferé vous envoyer ça plutôt que le copier coller d'un autre code.
      */
      ...
      for(i=0; i<4; i++);
      {
        	this.offscreenCanvas[i] = document.createElement("canvas");
			this.offscreenCanvas[i].width = this.width;
			this.offscreenCanvas[i].height = this.height;
			this.offscreenContext = this.offscreenCanvas[i].getContext("2d");
			this.offscreenContext.drawImage(this.img, 0, i*player.height, player.width, player.height, player.width, 0, 0, player.height, player.width);
      }
      /*this.offscreenCanvas1 = document.createElement("canvas");
		this.offscreenCanvas1.width = this.width;
		this.offscreenCanvas1.height = this.height;
		this.offscreenContext1 = this.offscreenCanvas1.getContext("2d");
		this.offscreenContext1.drawImage(this.img, 0, this.height, this.width, this.height, this.width, 0, 0, this.height, this.width);
		this.offscreenCanvas2 = document.createElement("canvas");
		this.offscreenCanvas2.width = this.width;
		this.offscreenCanvas2.height = this.height;
		this.offscreenContext2 = this.offscreenCanvas1.getContext("2d");
		this.offscreenContext2.drawImage(this.img, 0, 2*this.height, this.width, this.height, this.width, 0, 0, this.height, this.width);
		this.offscreenCanvas3 = document.createElement("canvas");
		this.offscreenCanvas3.width = this.width;
		this.offscreenCanvas3.height = this.height;
		this.offscreenContext3 = this.offscreenCanvas1.getContext("2d");
		this.offscreenContext3.drawImage(this.img, 0, 3*this.height, this.width, this.height, this.width, 0, 0, this.height, this.width);
		this.offscreenCanvas4 = document.createElement("canvas");
		this.offscreenCanvas4.width = this.width;
		this.offscreenCanvas4.height = this.height;
		this.offscreenContext4 = this.offscreenCanvas1.getContext("2d");
		this.offscreenContext4.drawImage(this.img, 0, 4*this.height, this.width, this.height, this.width, 0, 0, this.height, this.width);*/
    },
    x : 20,
    ySpeed : 10,
    y : 100,
    ...
    draw : function(){
        if(this.timeToBeAlive == 0) {
            conArena.drawImage(this.offscreenCanvas[this.cpt], 0,this.cpt*this.height,this.width,this.height, this.x,this.y,this.width,this.height);
        }else{
            //exploding
            if(this.cptExplosion!=0){
                conArena.drawImage(this.imgExplosion, this.cptExplosion*this.imgExplosionWidth, 0, this.imgExplosionWidth,this.imgExplosionHeight, this.x,this.y,this.width,this.height);
